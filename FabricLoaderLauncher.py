...
BASE_DIR = "D:\\TNTLauncher" if os.path.exists("D:\\") else "C:\\TNTLauncher"
LAUNCHER_DATA = os.path.join(BASE_DIR, "launcher_data.json")

def get_java_path():
    if not os.path.exists(LAUNCHER_DATA):
        raise RuntimeError("launcher_data.json not found")

    with open(LAUNCHER_DATA, "r", encoding="utf-8") as f:
        data = json.load(f)

    java_path = data.get("javaPath")

    if not java_path:
        raise RuntimeError("javaPath missing in launcher_data.json")

    if not os.path.exists(java_path):
        raise RuntimeError(f"Java not found at: {java_path}")
